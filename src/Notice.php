...
class Notice {
	...
	public function hooks(){
		add_action( 'admin_enqueue_scripts', 'enqueue_scripts', 99 );
		add_action( 'plugins_loaded', 'collect' );
		add_action( 'admin_notices', 'print' );
	}

	public function enqueue_scripts() {
		wp_enqueue_style( 'pluggable-product-notice', plugins_url( 'assets/css/notice.css', __FILE__ ), [], $this->plugin['Version'] );
	}
	...
	public function collect() {
		...
		if( isset( $this->plugin['min_wp'] ) && version_compare( get_bloginfo( 'version' ), $this->plugin['min_wp'], '<' ) ) {
			$notice = '<p>' . sprintf( __( '<strong>%s</strong> requires <i>WordPress version %s</i> or higher. You have <i>version %s</i> installed.', 'pluggable' ), $this->name, $this->plugin['min_wp'], get_bloginfo( 'version' ) ) . '</p>';
			self::add( $notice );
		}

		if( isset( $this->plugin['min_php'] ) && version_compare( PHP_VERSION, $this->plugin['min_php'], '<' ) ) {
			$notice = '<p>' . sprintf( __( '<strong>%s</strong> requires <i>PHP version %s</i> or higher. You have <i>version %s</i> installed.', 'pluggable' ), $this->name, $this->plugin['min_php'], PHP_VERSION ) . '</p>';
			self::add( $notice );
		}
		...
		foreach ( $this->plugin['depends'] as $plugin => $plugin_name ) {
			if( !in_array( $plugin, $active_plugins ) ) {
				...
				$button_text = array_key_exists( $plugin, $installed_plugins ) ? __( 'Activate', 'pluggable' ) : __( 'Install', 'pluggable' );
				$action_link = array_key_exists( $plugin, $installed_plugins ) ? $action_links['activate'] : $action_links['install'];
			
				$notice = '<p>' . sprintf( __( '<strong>ALERT:</strong> In order to <strong>%1$s</strong> run properly, <strong>%2$s</strong> needs to be activated. <a href="%3$s">%4$s %2$s Now</a>.', 'pluggable' ), $this->name, $plugin_name, $action_link, $button_text ) . '</p>';
	...
	public function print() {
		...
		if( did_action( 'pl-notice' ) ) return; // don't print notices more than once
		do_action( 'pl-notice' );

		global $pl_notices;

		if( is_array( $pl_notices ) && count( $pl_notices ) ) {
			foreach( $pl_notices as $notice ) {
				$is_dismissible = isset( $notice['dismissible'] ) && $notice['dismissible'] ? ' is-dismissible': '';
				echo "
					<div class='notice notice-{$notice['type']}{$is_dismissible} pl-notice pl-shadow'>
						" . $notice['text'] . "
					</div>
				";
			}
